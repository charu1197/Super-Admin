...
function sendVerificationEmail($user, $connection)
{
    $verificationCode = rand(10000, 99999);

    // Use pg_prepare to prepare the SQL statement
    $stmtUpdateCode = pg_prepare($connection, 'update_code_query', 'UPDATE super_admin_users SET request_code = $1 WHERE email = $2');
    
    // Use pg_execute to execute the prepared statement
    pg_execute($connection, 'update_code_query', array($verificationCode, $user['email']));

    // Save the verification code in the session
    $_SESSION['request_code'] = password_hash($verificationCode, PASSWORD_DEFAULT); // Updated this line

    // Send email with the verification code
    $subject = ucfirst($user['lastname']) . ', here\'s your request verification code:';
      ...
      <div style="background-color: #f0f6fb; padding: 3%;">
        ...
        <div style="padding: 0 0 3% 0;">
          ...
          <div style="margin: 3%;">
            ...
            <p style="margin: 3%; font-style: normal;">We received a request to reset the password on your PCC Account.</p>
            <h1 style="margin: 3%; font-style: normal;"> ' . $verificationCode . ' </h1>
            <p style="margin: 3%; font-style: normal;">Enter this code to complete the request change password process.</p>
